...
int main(int argc, char *argv[]) {
    ...
    if (!to_user || strlen(to_user) == 0) {
        fprintf(stderr, "Usage: %s <config> <video_path> [to_user_id]\n", argv[0]);
    ...
    printf("Sending video %s to %s\n", file_path, to_user);
    IlinkError err = ilink_bot_send_video(bot, to_user, file_path);
    printf("Video send result: %d (%s)\n", err, ilink_error_str(err));
